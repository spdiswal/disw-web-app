...
. In the `main/profile/content` folder, save the profile picture as a https://developers.google.com/speed/webp/docs/using[WebP] image file named `my-profile.webp`.
. In the same folder, create a TypeScript file named `my-profile.ts` and import the profile picture:
+
[source,typescript]
----
import pictureAssetUrl from "./my-profile.webp"
----
. Call the `defineProfileContent` function to describe the profile content and set the profile picture.
. Export the result in a constant named `content`.
