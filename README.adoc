...
Insert this content into the file to override the `VITE_PROFILE_NAME` environment variable:
+
[source,properties]
----
...
. A *server-oriented build* which produces a CommonJS module that generates a static HTML tree from the initial state of the web app.
The entry point is `main-server.tsx`.
Vite operates in `ssr` mode during this phase.

. A *client-oriented build* which pre-renders the HTML page and produces a browser script that makes the web app interactive by hydrating the HTML page.
...
_Hydration_ is a process carried out by the web framework (e.g. Preact) that attaches event listeners to the pre- or server-side-rendered HTML document and constructs a reactive component tree equivalent to it.
====
